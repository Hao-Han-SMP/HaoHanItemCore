...
public final class DefaultItemFactory implements ItemFactory {
    ...
    public ItemStack create(ItemDefinition definition, int amount) {
        ...
        if (!definition.getLore().isEmpty()) {
            List<Component> loreComponents = definition.getLore().stream()
        ...
        applyComponents(meta, definition, itemStack, plugin);
        ...
        if (customBlockData instanceof String blockDataStr) {
            itemStack = applyBlockStateComponent(itemStack, definition.getId(), blockDataStr);
        }
     ...
     */
    public static boolean applyComponents(ItemMeta meta, ItemDefinition definition, ItemStack itemStack, Plugin plugin) {
        boolean modified = false;

        // 1. Item Model (Paper 1.21+)
        String model = definition.getItemModel();
        if (model == null && definition.getCustomModelData() != null) {
            model = definition.getId();
        }
    ...
    private ItemStack applyBlockStateComponent(ItemStack itemStack, String id, String blockDataStr) {
        try {
            ...
            // 5. Instantiate BlockItemStateProperties using reflection
            ...
            // 6. Set component on NMS ItemStack
